...
private fun Maze7.countAliveNeighbors(x: Int, y: Int): Int {
  var count = 0
...
fun Maze7.forEachAlive(f: (x: Int, y: Int) -> Unit) {
  for (y in 0 until height) {
    for (x in 0 until width) {
...
fun Maze7.toSize(width: Int = this.width, height: Int = this.height): Maze3 {
